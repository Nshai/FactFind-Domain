...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SpCustomRetrieveUserSession]
...
BEGIN
  SELECT
    ...
    ISNULL(T2.Recent, '') AS [User!1!Recent], 
    ISNULL(T2.RecentWork, '') AS [User!1!RecentWork], 
    T2.ConcurrencyId AS [User!1!SessionConcurrencyId]
  FROM 
	TUser T1 WITH(NOLOCK)
	JOIN TUserSession T2 WITH(NOLOCK) ON T2.UserId = T1.UserId
  WHERE 
	T2.SessionId = @SessionId
