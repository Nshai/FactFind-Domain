...
SET ANSI_NULLS ON
GO
CREATE Procedure [dbo].[nio_SpCustomUpdateGroupVATNumber]
	(
		@GroupId bigint,
...
SET @TenantId = (select IndigoClientId FROM TGroup WHERE GroupId = @GroupId)

IF @SaveOption = 1 --this group only
BEGIN
	--audit
	INSERT INTO TGroupAudit (Identifier,GroupingId,IndigoClientId,CRMContactId,ParentId,LegalEntity,GroupImageLocation,AcknowledgementsLocation,FinancialYearEnd,ApplyFactFindBranding,VatRegNbr,FSARegNbr,ConcurrencyId,GroupId,StampAction,StampDateTime,StampUser)
	SELECT Identifier,GroupingId,IndigoClientId,CRMContactId,ParentId,LegalEntity,GroupImageLocation,AcknowledgementsLocation,FinancialYearEnd,ApplyFactFindBranding,VatRegNbr,FSARegNbr,ConcurrencyId,GroupId,'U',getdate(), @StampUser
	FROM TGroup 
...
BEGIN
	...
	--audit
	INSERT INTO TGroupAudit (Identifier,GroupingId,IndigoClientId,CRMContactId,ParentId,LegalEntity,GroupImageLocation,AcknowledgementsLocation,FinancialYearEnd,ApplyFactFindBranding,VatRegNbr,FSARegNbr,ConcurrencyId,GroupId,StampAction,StampDateTime,StampUser)
	SELECT g.Identifier,GroupingId,IndigoClientId,CRMContactId,ParentId,LegalEntity,GroupImageLocation,AcknowledgementsLocation,FinancialYearEnd,ApplyFactFindBranding,VatRegNbr,FSARegNbr,ConcurrencyId,g.GroupId,'U',getdate(), @StampUser
	FROM TGroup g
