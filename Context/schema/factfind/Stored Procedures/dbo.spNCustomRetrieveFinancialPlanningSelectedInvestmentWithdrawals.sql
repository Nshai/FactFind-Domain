...
SET ANSI_NULLS ON
GO
create procedure [dbo].[spNCustomRetrieveFinancialPlanningSelectedInvestmentWithdrawals]
@FInancialPlanningId bigint

as

select pd.investmentid,
		case
...
inner join TFinancialPlanningSelectedInvestments pd on pd.investmentid = mi.PolicyBusinessId
where	pd.FinancialPlanningId = @FinancialPlanningId and
		InvestmentType != 'Asset' and
		rf.reffrequencyid in (1,2,3,4,5,7,8) and
		paymentstartdate < getdate() and
		(paymentstopdate is null or paymentstopdate > getdate()) 
group by pd.investmentid,
		case
		when EscalationType is null then 'NONE'
		when EscalationType = 'Fixed' then 'NONE'
		when EscalationType = 'RPI' then 'RPI'
