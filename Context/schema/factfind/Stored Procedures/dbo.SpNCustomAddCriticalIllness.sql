...
IF @CriticalIllnessId1 IS NULL AND (LEFT(@LifeAssured,5) = 'Joint' or @LifeAssured = 'Client 1')
	BEGIN
...
ELSE
	BEGIN
		...
		SET 
			Amount = @CriticalIllnessAmount,
			ConcurrencyId = ConcurrencyId + 1
		WHERE 
			CriticalIllnessId = @CriticalIllnessId1
	END


IF @CriticalIllnessId2 IS NULL AND (LEFT(@LifeAssured,5) = 'Joint' or @LifeAssured = 'Client 2')
...
ELSE
	BEGIN
		...
		SET 
			...
			ConcurrencyId = ConcurrencyId + 1
		WHERE 
			CriticalIllnessId = @CriticalIllnessId2
	END
