...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[spNCustomRetrievePlanSimple]        
...
SELECT           distinct         
 ...
 own2.firstname + ' ' + own2.lastname as owner2name,          
  ...
  pb.ProductName,                             
  pb.SequentialRef,      
  case when   pts.section in ('Final Salary Schemes','Money Purchase Pension Schemes') then 'pension' else 'investment' end as agreementType,      
  isnull(reservedvalue,0) as reservedvalue,      
  cast(isnull(ext.annualcharges,0) as varchar) as AnnualCharge,      
cast(isnull(ext.WrapperCharge,0) as varchar) as WrapperCharge,      
cast(isnull(ext.InitialAdviceCharge,0) as varchar) as InitialCharge,      
cast(isnull(ext.OngoingAdviceCharge,0) as varchar) as OngoingCharge,
[Status].IntelligentOfficeStatusType    AS PlanStatus
