...
SET ANSI_NULLS ON
GO
CREATE Procedure [dbo].[nio_SpCustomSearchClientFullSecureByAdviceCase]
	(
		@TenantId bigint,    
...
Where C.IndigoClientId = @TenantId        
...
CASE T1._OwnerId        
 ...
 FROM     
  ...
  (    
   ...
   FROM    
    ...
    -- JOIN PolicyManagement..TPolicyBusiness T7  WITH(NOLOCK)  ON T3.PolicyDetailId = T7.PolicyDetailId AND T7.IndigoClientId = @TenantId    
    ...
    JOIN PolicyManagement..TStatus T11  WITH(NOLOCK)  ON T11.StatusId = T10.StatusId AND ISNULL(T11.IntelligentOfficeStatusType, '') != 'Deleted'    
        And T11.IndigoClientId = @TenantId    
   WHERE    
 ...
 WHERE     
  T1.IndClientId = @TenantId AND ISNULL(T1.RefCRMContactStatusId,0) = 1 AND T1.ArchiveFG=0        
...
LEFT JOIN     
  ...
  Where defaultfg = 1  and A.indclientid = @TenantId      
