...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SpUpdateContactByIdAndConcurrencyId]
@KeyContactId bigint,
...
BEGIN
  ...
  WHERE (T1.ContactId = @KeyContactId) AND 
...
SELECT * FROM TContact [Contact]
  WHERE ([Contact].ContactId = @KeyContactId) AND 
        ([Contact].ConcurrencyId = @KeyConcurrencyId + 1)
 FOR XML AUTO
