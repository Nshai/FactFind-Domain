...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SpCustomSearchClientFullSecureByFeeExport]    
...
BEGIN    
 SET @SequentialRef='IOF' + RIGHT(REPLICATE('0',8) + @SequentialRef,8)    
END    
...
ISNULL(T1.PersonId, '') AS [CRMContact!1!PersonId],   
 ...
 ISNULL(T1.TrustId, '') AS [CRMContact!1!TrustId],      
  ...
  T1.LastName AS [CRMContact!1!LastName],     
  T1.FirstName AS [CRMContact!1!FirstName],     
 ...
 ISNULL(TF.SequentialRef, '') AS [CRMContact!1!IOFReference],  
 ISNULL(TF.FeeId,0) AS [CRMContact!1!FeeId]     
    
     
    

   From PolicyManagement..TFee TF WITH(NOLOCK)    
   JOIN PolicyManagement..TFeeRetainerOwner TFRO WITH(NOLOCK) ON TF.FeeId=TFRO.FeeId    
  ...
  LEFT JOIN TContact T23 WITH(NOLOCK) ON T23.CRMContactId = T1.CRMContactId AND T23.DefaultFG = 1 AND T23.RefContactType = 'Web Site'
   ...
   WHERE TF.IndigoClientId=@IndigoClientId    
   AND TFRO.IndigoClientId=@IndigoClientId    
   AND TF.SequentialRef=@SequentialRef    
