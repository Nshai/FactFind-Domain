...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SpCustomSearchClientFullExportSecure]  
...
--if we are searching on fund stuff then limit results to clients with plans  
if(@FundUnitId is not null or @NonFeedFundId is not null or @RefFundManager is not null) begin  
 select @IsLimited = 1  
end  
...
Begin    
  IF @SequentialRefType='PlanIOBRef'    
	  BEGIN    
	   exec dbo.SpCustomSearchClientFullSecureByPolicyNoExport @IndigoClientId, '', @SequentialRef, @_UserId, @_TopN    
	  END    
  ELSE IF @SequentialRefType='FeeIOFRef'    
	  BEGIN    
	   exec dbo.SpCustomSearchClientFullSecureByFeeExport @IndigoClientId, @SequentialRef, @_UserId, @_TopN    
	  END    
  ELSE IF @SequentialRefType= 'RetainerIORRef'    
	  BEGIN    
	   exec dbo.SpCustomSearchClientFullSecureByRetainerExport @IndigoClientId, @SequentialRef, @_UserId, @_TopN    
	  END
  ELSE IF  @SequentialRefType= 'AdviceCaseRef'    
	 BEGIN 
	   exec dbo.SpCustomSearchClientFullSecureByAdviceCaseExport @IndigoClientId, @SequentialRef, default,@_UserId, @_TopN    
	 END   
End 
...
Begin  
 exec dbo.SpCustomSearchClientFullSecureByAdviceCaseExport @IndigoClientId,default, @AdviceCaseName, @_UserId, @_TopN     
End 
ELSE 
BEGIN
...
FROM   
 ...
 (  
  ...
  WHERE  
   ...
   AND (@PlanTypeId = 0 OR T6.RefPlanTypeId = @PlanTypeId)  
   AND (@PolicyNumber IS NULL OR T7.PolicyNumber LIKE @PolicyNumber)  
   AND (@ProductName IS NULL OR T7.ProductName LIKE @ProductName +'%')  
   AND (@RefProdProviderId = 0 OR T8.RefProdProviderId = @RefProdProviderId)  
   AND (@RefFundManager IS NULL OR (F.FundProviderId = @RefFundManager OR Nff.CompanyId = @RefFundManager))    
   AND (@FundUnitId  IS NULL OR (fu.FundUnitId = @FundUnitId))  
   AND (@NonFeedFundId  IS NULL OR (Nff.NonFeedFundId = @NonFeedFundId))  
...
WHERE   
 ...
 AND (@PractitionerId = 0 OR T12.PractitionerId = @PractitionerId)  
 ...
 AND (@PlanTypeId = 0 OR Plans.RefPlanTypeId = @PlanTypeId)  
 AND (@PolicyNumber IS NULL OR Plans.PolicyNumber LIKE @PolicyNumber)  
 AND (@ProductName IS NULL OR Plans.ProductName LIKE @ProductName +'%')  
 AND (@RefProdProviderId = 0 OR Plans.RefProdProviderId = @RefProdProviderId)  
 ...
 AND (T14.Postcode like @Postcode + '%' OR @Postcode IS NULL)  
