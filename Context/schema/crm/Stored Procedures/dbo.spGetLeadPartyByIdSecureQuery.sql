...
-- Description:	Stored procedure for getting Lead party by Id
-- =============================================
CREATE PROCEDURE [dbo].[spGetLeadPartyByIdSecureQuery]
    @partyId INT,
    @userId INT,
    @tenantId INT
AS
BEGIN
    ...
    SET @DatabaseEntityTypeName = N'Lead' --for Lead Party
    ...
    INNER JOIN @AccessRightsByEntityTypeTemp accessRights ON accessRights.EntityId = @partyId
    INNER JOIN TLead lead ON lead.CRMContactId = contact.CRMContactId
    WHERE contact.CRMContactId = @partyId
    AND contact.RefCRMContactStatusId = 2 --'Prospect' = 'Lead'
END
GO
