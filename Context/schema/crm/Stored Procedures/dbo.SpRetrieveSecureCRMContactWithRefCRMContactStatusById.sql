...
SET ANSI_NULLS OFF
GO
CREATE PROCEDURE [dbo].[SpRetrieveSecureCRMContactWithRefCRMContactStatusById]
@CRMContactId bigint,
@_UserId bigint
AS
...
BEGIN
  ...
  SELECT
    ...
    NULL AS [RefCRMContactStatus!2!RefCRMContactStatusId], 
    NULL AS [RefCRMContactStatus!2!StatusName], 
    NULL AS [RefCRMContactStatus!2!OrderNo], 
    NULL AS [RefCRMContactStatus!2!InternalFG], 
    NULL AS [RefCRMContactStatus!2!ConcurrencyId]
  FROM TCRMContact T1
      -- Secure clause
  ...
  SELECT
    ...
    ISNULL(T2.StatusName, ''), 
    ISNULL(T2.OrderNo, ''), 
    ISNULL(T2.InternalFG, ''), 
    T2.ConcurrencyId
  FROM TRefCRMContactStatus T2
  INNER JOIN TCRMContact T1
  ON T2.RefCRMContactStatusId = T1.RefCRMContactStatusId
      -- Secure clause
      LEFT JOIN VwCRMContactKeyByCreatorId TCKey ON TCKey.UserId = @_UserId AND TCKey.CreatorId = T1._OwnerId
      LEFT JOIN VwCRMContactKeyByEntityId TEKey ON TEKey.UserId = @_UserId AND TEKey.EntityId = T1.CRMContactId
  WHERE (T1.CRMContactId = @CRMContactId)
        AND (@_UserId < 0 OR (T1._OwnerId=@_UserId OR (TCKey.CreatorId IS NOT NULL OR TEKey.EntityId IS NOT NULL)))

  ORDER BY [CRMContact!1!CRMContactId], [RefCRMContactStatus!2!RefCRMContactStatusId]
