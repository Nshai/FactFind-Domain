...
CREATE PROCEDURE [dbo].[nio_SearchContactTypeBy_CN_CT]    
@TenantId BIGINT,
@Name VARCHAR (50),
...
SELECT   
 ...
 T4.RefCorporateTypeId AS TypeId,  
 T4.TypeName AS [Type],
...
FROM TCRMContact T1  
 ...
 INNER JOIN TCorporate T3  
  ON T3.CorporateId = T1.CorporateId  
 LEFT JOIN TRefCorporateType T4  
  ON T4.RefCorporateTypeId = T3.RefCorporateTypeId  
...
WHERE (T1.IndClientId = @TenantId)   
  ...
  AND (@TypeId IS NULL OR T3.RefCorporateTypeId = @TypeId)      
