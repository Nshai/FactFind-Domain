...
-- Superseded by dbo.spGetUserClaimsByPartyIdAndReach.
-- Remove once dbo.spGetUserClaimsByPartyIdAndReach is deployed and there are no usages for it

CREATE procedure [dbo].[spGetUserClaimsByPartyId]
@PartyId int,
@CallerUserId int
as
...
callingUser.ActiveRole as 'RoleId', 
...
join administration..TUser callingUser on callingUser.UserId = @CallerUserId
inner join administration..TIndigoClient t on callingUser.IndigoClientId = t.IndigoClientId
...
LEFT JOIN VwCRMContactKeyByEntityId TEKey ON TEKey.UserId = @CallerUserId AND TEKey.EntityId = findParty.CRMContactId  -- Note 
...
WHERE findParty.CRMContactId = @PartyId
and (callingUser.SuperUser = 1 OR callingUser.SuperViewer = 1 OR findUser.UserId=@CallerUserId OR (findParty._OwnerId=@CallerUserId OR (TCKey.CreatorId IS NOT NULL OR TEKey.EntityId IS NOT NULL)))
