...
CREATE PROCEDURE [dbo].[spCustomTransitionRule_LatestOpportunityCampaignType]
...
AS
	...
	--  now get the most recent opportunity across all those clients
	...
	INNER JOIN CRM..TCampaignData cd ON cd.CampaignDataId = o.CampaignDataId
	INNER JOIN CRM..TCampaign ca ON ca.CampaignId = cd.CampaignId
	WHERE ca.CampaignTypeId IS NULL
	...
	BEGIN
		...
		SELECT @ErrorMessage = 'OPPCAMPAIGNTYPE_' + @Ids
