...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SpPlansExistForUserByPlanIds]
@UserId int,
@TenantId int,
...
BEGIN
	...
	IF NOT EXISTS (SELECT 1 FROM @PlanSet s LEFT JOIN @UserPlans p ON s.VALUE = p.PlanId WHERE p.PlanId IS NULL)
	  SET @AllValid = 1
