...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SpDashboardRetrieveClientPortfolioForProtection]
	@TenantId bigint,
	@cid bigint
AS
...
FROM
	...
	JOIN TStatus S ON S.StatusId = Sh.StatusId AND s.IndigoClientId=@TenantId 			
WHERE
	PO.CRMContactId = @cid AND S.IntelligentOfficeStatusType IN ('In Force', 'Paid Up')
...
SELECT 
		...
		CASE 
			...
			WHEN planTypes.PlanTypeName = 'Pension Term Assurance' THEN CASE WHEN Fund.FundValue IS NOT NULL OR Assets.Amount IS NOT NULL THEN ISNULL((Fund.FundValue * R.Rate), 0) + ISNULL(Assets.Amount, 0) ELSE COALESCE((LastVal.PlanValue * R.Rate), (PT.LifeCoverSumAssured * R.Rate), 0) END
			-- Special case for PHI
			WHEN planTypes.PlanTypeName LIKE 'Permanent Health%' THEN COALESCE((B.BenefitAmount * R.Rate), (PT.LifeCoverSumAssured * R.Rate), 0)
			-- Other plans by category.
			ELSE COALESCE((PT.LifeCoverSumAssured * R.Rate), (GI.SumAssured * R.Rate), 0)
		END as Value,
		CASE WHEN AssetAlternativeCurrency = 1 OR pb.CurrencyCode != @RegionalCurrency THEN 1 ELSE 0 END as AlternativeCurrency
	INTO #Result
	FROM 
		@Plans pb
