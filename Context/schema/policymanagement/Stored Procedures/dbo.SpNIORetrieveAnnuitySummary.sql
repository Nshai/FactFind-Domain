...
SET ANSI_NULLS ON;
GO
CREATE PROCEDURE [dbo].[SpNIORetrieveAnnuitySummary] @PolicyBusinessId BIGINT,@TenantId BIGINT,@ProductTypeId BIGINT
AS
     BEGIN
         ...
         SELECT pp.protectionid,
                ...
                pp.InTrust,
                pp.ToWhom,
                lives.AssuredLife1Id,
                lives.AssuredLife2Id,
                al1.partyid AS lifeassured1partyid,
                al2.partyid AS lifeassured2partyid,
                ...
                b1.PensionCommencementLumpSum,
                b1.IsSpousesBenefit,
                b1.SpousesOrDependentsPercentage,
                b1.IsProportion,
                b1.PCLSPaidById,
                b1.IsCapitalValueProtected,
                b1.CapitalValueProtectedAmount,
                b1.GADMaximumIncomeLimit,
                b1.GADCalculationDate,
                b1.GuaranteedMinimumIncome,
                b1.LumpSumDeathBenefitAmount,
                b1.GuaranteedPeriod,
			 ...
			 PExt.WhoCreatedUserId,
			 PExt.FundIncome,
			 PExt.AdditionalNotes,
			 PInfo.EnhancedTaxFreeCash,
			 PInfo.guaranteedAnnuityRate,
			 PInfo.applicablePenalties,
			 PInfo.efiLoyaltyTerminalBonus,
			 PInfo.guaranteedGrowthRate,
			 PInfo.optionsAvailableatRetirement,
			 PInfo.otherBenefitsAndMaterialFeatures,
			 PInfo.LifetimeAllowanceUsed,
			 Pinfo.DeathBenefit,
			 CAST(PExt.IsTargetMarket as INT) AS TargetMarketId,
			 PExt.TargetMarketExplanation,
			 Pinfo.CrystallisationStatus,
	         Pinfo.HistoricalCrystallisedPercentage,
	         Pinfo.CurrentCrystallisedPercentage,
	         Pinfo.CrystallisedPercentage,
	         Pinfo.UncrystallisedPercentage,
	         Pinfo.PensionArrangement,
			 Pinfo.PensionSharingPercentage,
			 Pinfo.PensionAttachmentOrder,
			 b1.IsPre75
           FROM policymanagement..tpolicybusiness p
                ...
                LEFT JOIN( 
                           SELECT AssuredLife1Id = MIN( a.AssuredLifeId ),
                                  AssuredLife2Id = CASE
                                                   WHEN MAX( a.AssuredLifeId ) = MIN( a.AssuredLifeId ) THEN NULL
                                                       ELSE MAX( a.AssuredLifeId )
                                                   END,
                                  b.PolicyBusinessId
                             FROM policymanagement..tAssuredLife a
                                  INNER JOIN policymanagement..tprotection b
                                    ON a.protectionid = b.protectionid
                             WHERE b.policybusinessid = @PolicyBusinessId
                             GROUP BY b.PolicyBusinessId ) AS lives
                  ON lives.PolicyBusinessId = P.PolicyBusinessId
                LEFT JOIN policymanagement..tAssuredLife al1
                  ON al1.AssuredLifeId = lives.AssuredLife1Id
                LEFT JOIN policymanagement..tbenefit b1
                  ON b1.benefitid = al1.benefitid
                LEFT JOIN policymanagement..tAssuredLife al2
                  ON al2.AssuredLifeId = lives.AssuredLife2Id
                LEFT JOIN policymanagement..tbenefit b2
                  ON b2.benefitid = al2.benefitid --Oppourtunity Reference - AME-1514      
