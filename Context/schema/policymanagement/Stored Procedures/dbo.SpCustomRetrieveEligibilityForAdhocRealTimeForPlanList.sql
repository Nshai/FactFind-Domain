...
CREATE procedure [dbo].[SpCustomRetrieveEligibilityForAdhocRealTimeForPlanList]
@PolicyBusinessIdList PolicyBusinessIdListType  READONLY,
@TenantId bigint,
@UserId bigint,
@Now datetime
as
...
begin
	...
	(
	   select pp.PolicyBusinessId, pp.ValuationProviderId, certf1.[Subject] as SellingAdviserCertificate, 
		...
		from @PolicyBusinessIdList m 
		join TValPotentialPlan pp on pp.PolicyBusinessId = m.PolicyBusinessId
		...
		where pp.IndigoClientId = @TenantId
	),
	...
	inforce(PolicyBusinessId) as
	(
		select m.PolicyBusinessId as PolicyBuisnessId	      
	   from @policyBusinessIdList m	
	   join TPolicyBusiness pb on m.PolicyBusinessId = pb.PolicyBusinessId
    ...
    (
		...
		from @PolicyBusinessIdList m
		join TPlanValuation val on val.PolicyBusinessId = m.PolicyBusinessId	
		...
		from @PolicyBusinessIdList m
		join credentials cred on cred.PolicyBusinessId = m.PolicyBusinessId
		where cred.LoggedInUserCertificate is null and cred.SellingAdviserCertificate is null and cred.ServicingAdviserCertificate is null
		...
		select  m.PolicyBusinessId as PolicyBusinessId, 
	       0 as IsEligible, 
		   103 as EligibilityCode
		from @policyBusinessIdList m
		join TValPotentialPlan pp on m.PolicyBusinessId = pp.PolicyBusinessId
		join TValProviderConfig c  on c.RefProdProviderId = pp.ValuationProviderId
		cross apply TRefPlanValueType  r
		where c.SupportedService & r.ServiceType <> r.ServiceType 
			and r.ServiceType = 4
			and pp.IndigoClientId = @TenantId
			
		union 

		--ineligible if not in tvalpotentialplan
		select  m.PolicyBusinessId as PolicyBusinessId, 
	       0 as IsEligible, 
		   105 as EligibilityCode
		from @policyBusinessIdList m 		
		where m.PolicyBusinessId not in 
		              (select PolicyBusinessId from TValPotentialPlan where m.PolicyBusinessId = PolicyBusinessId and IndigoClientId = @TenantId) 				
	   ...
	   select m.PolicyBusinessId as PolicyBuisnessId,
	      0 as IsEligible,
	      2 as EligibilityCode
	   from @policyBusinessIdList m	
	   where m.PolicyBusinessId not in (select PolicyBusinessId from inforce)              
	)

insert into @eligibles(policyBusinessId)
    select PolicyBusinessId from @policyBusinessIdList
    except
