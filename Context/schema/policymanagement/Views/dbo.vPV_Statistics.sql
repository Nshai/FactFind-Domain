create view [dbo].[vPV_Statistics]
as
--Plan statistics Total Plans | Potential Plans | Valued Plans
select A.IndigoClientId,RefProdProviderId,ValuationProviderId,RefPLanTypeId,count(PolicyBusinessID) as TotalPlans, count(ValPotentialPlanId) as PotentialPlans, count(PlanValuationId) as ValuedPlans,convert(date,getdate()) dated,DIFlag
from    (
...
group by A.IndigoClientId,RefProdProviderId,ValuationProviderId,RefPLanTypeId,DIFlag
go